...
export class Project {
    ...
    constructor(repository: Repository) {
        ...
        this.repo_path = path.resolve(resolveHome(APP_CONFIG.deploy_url), `${repository.name}-${repository.id}`);
        this.config = new ProjectConfig(this.repo_path)
    }

    exec(cmd?: string, deployment?: Deployment) {
        if(!cmd) return;
        child_process.execSync(cmd, {cwd: path.resolve(this.repo_path, deployment?.path ?? "")})
    }

    updateRepo() {
        if (!fs.existsSync(this.repo_path)) {
            console.log(`Cloning repository ${this.repository.full_name} to ${this.repo_path}`)

            child_process.execSync(`git clone '${this.repository.ssh_url}' '${this.repo_path}'`)
